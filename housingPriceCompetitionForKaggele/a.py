...
best_mae = 1e9;
best_leaf_node = 0;
for i in range(2,500,50): #MaxLeafNode >= 2
	#print(i,get_mae(i,train_X,val_X,train_y,val_y));
	cur_mae = get_mae(i,train_X,val_X,train_y,val_y);
	if( cur_mae < best_mae ):
		best_mae = cur_mae;
...
print( 'Best Mae:', best_mae, 'Best Leaf Node:',best_leaf_node );
...
best_mae = 1e9;
best_leaf_node = 0;
for i in range(400,600,100): #MaxLeafNode >= 2
	#print(i,get_mae(i,train_X,val_X,train_y,val_y));
	cur_mae = get_mae(i,train_X,val_X,train_y,val_y);
	if( cur_mae < best_mae ):
		best_mae = cur_mae;
...
best_mae = 1e9;
best_leaf_nodes = 1e9;
for max_leaf_nodes in range(600,800,50):
...
from sklearn.model_selection import train_test_split;
train_X, val_X, train_y, val_y = train_test_split(X, y, random_state = 0);
cols_with_missing = [ col for col in train_X.columns if train_X[col].isnull().any() ]; # train_X[col].isnull().any() or X[col].isnull().any()  
...
#Remove Rows(axis=0:dropRow,axis=1:dropCol) form missing traget['SalePrice']
...
print(missing_val_count_by_column_X_given);
print(list(X_given.columns),sep='\n');
...
best_mae = 1e9;
best_nodes = 1e9;
for cur_nodes in range(400,600,1):
